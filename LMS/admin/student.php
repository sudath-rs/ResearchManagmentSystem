...
if(isset($_SESSION['login_user']))
{
  ...
  {
    $q=mysqli_query($db,"SELECT id,username,firstname,lastname,institute,department,email,contact FROM `student` where `username` like '%$_POST[search]%'");
...
{
  $res=mysqli_query($db,"SELECT id,username,firstname,lastname,institute,department,email,contact FROM `student` ORDER BY `student`.`firstname` ASC");
  echo "<table class='table table-bordered table-hover'>";
  echo "<tr style='background-color:#ceff00;'>";
  echo "<th>"; echo "No"; echo "</th>";
  echo "<th>"; echo "StudentID"; echo "</th>";
